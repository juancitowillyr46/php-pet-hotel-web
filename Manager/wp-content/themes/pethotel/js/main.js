(function($) {
	...
	var fileSizeMax = 2097152; // 1813346
	...
	$(document).on('change', '.ms-picture-file', function(e) {
		...
		if(file_size > fileSizeMax) { 
			...
			$(".error-size").show().html('El tamaño del archivo es superior a 2 MB');
	...
	$(document).on('click', '#btnCropp', function(e) {
		...
		if($(avatar).closest('form').find("input[name='screenVoucher']").length > 0) {
			...
			inputImage = 'screenVoucher';



		} else if($(avatar).closest('form').find("input[name='image']").length > 0) {
		...
		canvas = cropper.getCroppedCanvas(croppedCanvas);

		avatar.attr("src",canvas.toDataURL());

		$(avatar).closest('form').find("input[name='"+inputImage+"']").val(canvas.toDataURL());

		$(avatar).closest('form').find(".error-size").hide().html('');
	...
	$(document).on('click', '#doggyBtn', function(e) {
		e.preventDefault();
		$("body").addClass('modal-open');
		$("#modal-doggy-school").fadeIn();
	});
